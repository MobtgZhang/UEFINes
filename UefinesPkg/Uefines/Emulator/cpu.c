...
//
// Addressing modes (merged from cpu-addressing.c for single TU of cpu-internal globals).
//
void
cpu_address_implied (
  void
  ...
  )
{
  ...
  ArgAddr = memory_readw (cpu.PC);
  if ((ArgAddr & 0xFF) == 0xFF) {
    op_address = (memory_readb (ArgAddr & 0xFF00) << 8) + memory_readb (ArgAddr);
  } else {
    op_address = memory_readw (ArgAddr);
  }
  ...
  )
{
  ...
  ArgAddr    = memory_readb (cpu.PC);
  op_address = (memory_readb ((ArgAddr + cpu.X + 1) & 0xFF) << 8) |
               memory_readb ((ArgAddr + cpu.X) & 0xFF);
  op_value   = memory_readb (op_address);
  cpu.PC++;
}
  ...
  )
{
  ...
  ArgAddr    = memory_readb (cpu.PC);
  op_address = (((memory_readb ((ArgAddr + 1) & 0xFF) << 8) | memory_readb (ArgAddr)) + cpu.Y) & 0xFFFF;
  op_value   = memory_readb (op_address);
  cpu.PC++;
  if ((op_address >> 8) != (cpu.PC >> 8)) {
    op_cycles++;
